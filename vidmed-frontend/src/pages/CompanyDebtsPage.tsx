...
import { companyDebtService, companyDebtPaymentService } from '../services/dataService';
import { CompanyDebt, CompanyDebtPayment } from '../types';
import { useAuthStore } from '../stores/authStore';

interface CompanyDebtFormData {
  company_name: string;
...
export const CompanyDebtsPage: React.FC = () => {
  ...
  const [editingDebt, setEditingDebt] = useState<CompanyDebt | null>(null);
  const [selectedDebt, setSelectedDebt] = useState<CompanyDebt | null>(null);
  const [viewDebt, setViewDebt] = useState<CompanyDebt | null>(null);
  const [payments, setPayments] = useState<CompanyDebtPayment[]>([]);

  const { control: debtControl, handleSubmit: handleDebtSubmit, reset: resetDebt, formState: { errors: debtErrors } } = useForm<CompanyDebtFormData>();
  ...
  const loadDebts = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await companyDebtService.list();
      setDebts(data);
    } catch (err: any) {
      setError(err.message || 'Erreur lors du chargement des dettes entreprise');
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadPayments = async (debtId: number) => {
    try {
      const data = await companyDebtPaymentService.list();
      setPayments(data.filter((p: CompanyDebtPayment) => p.debt === debtId));
    } catch (err: any) {
  ...
  const handleOpenDebtDialog = (debt?: CompanyDebt) => {
    if (debt) {
      setEditingDebt(debt);
      resetDebt({
        company_name: debt.company_name,
        company_phone: debt.company_phone,
  ...
  const handleOpenPaymentDialog = (debt: CompanyDebt) => {
  ...
  const onDebtSubmit = async (data: CompanyDebtFormData) => {
    try {
      setError(null);
      const payload = {
        ...data,
      ...
      if (editingDebt) {
        await companyDebtService.update(editingDebt.id, payload);
      } else {
        await companyDebtService.create(payload);
      }
  ...
  const onPaymentSubmit = async (data: PaymentFormData) => {
    ...
    try {
      ...
      await companyDebtPaymentService.create(payload);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      setError(null);
      await companyDebtService.delete(id);
      loadDebts();
    } catch (err: any) {
  ...
  const handleView = async (debt: CompanyDebt) => {
    setViewDebt(debt);
  ...
  const getStatusChip = (debt: CompanyDebt) => {
    if (debt.is_paid) {
  ...
  const getPaymentProgress = (debt: CompanyDebt) => {
  ...
  return (
    <Box>
            ...
            {debts.length === 0 ? (
              <TableRow>
                <TableCell colSpan={8} align="center">
                  <Typography variant="body2" color="text.secondary">
                    Aucune dette entreprise disponible
                  </Typography>
                </TableCell>
              </TableRow>
            ) : (
              debts.map((debt) => (
                <TableRow key={debt.id} hover>
                  ...
                  <TableCell>{debt.company_phone}</TableCell>
      ...
      <Dialog open={openDebtDialog} onClose={handleCloseDebtDialog} maxWidth="md" fullWidth>
        <DialogTitle>
          {editingDebt ? 'Modifier la Dette' : 'Nouvelle Dette Entreprise'}
        </DialogTitle>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <TextField
                      ...
                      fullWidth
                      error={!!debtErrors.company_name}
                      helperText={debtErrors.company_name?.message}
                    />
                  )}
                />
              </Grid>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <TextField
                      ...
                      label="Téléphone"
                      fullWidth
                      error={!!debtErrors.company_phone}
                      helperText={debtErrors.company_phone?.message}
                    />
                  )}
                />
              </Grid>
        ...
        <form onSubmit={handlePaymentSubmit(onPaymentSubmit)}>
          <DialogContent>
            {selectedDebt && (
              <Box mb={2}>
                <Typography variant="body2" color="text.secondary">
                  Entreprise: {selectedDebt.company_name}
                </Typography>
                <Typography variant="body2" color="text.secondary">
      ...
      <Dialog open={!!viewDebt} onClose={() => setViewDebt(null)} maxWidth="md" fullWidth>
        <DialogTitle>Détails de la Dette Entreprise</DialogTitle>
        <DialogContent>
          {viewDebt && (
            <Box>
              <Grid container spacing={2} mb={3}>
                <Grid item xs={6}>
                  ...
                  <Typography variant="body1">{viewDebt.company_name}</Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="body2" color="text.secondary">
                    Téléphone
                  </Typography>
                  <Typography variant="body1">{viewDebt.company_phone}</Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="body2" color="text.secondary">
                    Montant Initial
